...
Parser output 應是 candidate facts、candidate 上的 entity references（ADR-022）與 evidence anchors。
...
- Low-confidence extraction 必須留在 `candidate_facts`，state 為 `needs_review`。
